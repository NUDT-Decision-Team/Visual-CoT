...
# 创建一个空字典，用于存储整合后的种族数据
knowledge_dict = {}

# 更新字典，将不同种族的数据（神族、虫族、人族）合并到 knowledge_dict 中
# protoss.DATA_PROTOSS、terran.DATA_TERRAN、zerg.DATA_ZERG 是不同种族的具体数据，包含单位、建筑、技能等信息
knowledge_dict.update(protoss.DATA_PROTOSS)  # 将神族的知识数据添加到字典
knowledge_dict.update(terran.DATA_TERRAN)  # 将人族的知识数据添加到字典
knowledge_dict.update(zerg.DATA_ZERG)     # 将虫族的知识数据添加到字典

# 创建一个字典，用于存储单位 ID 到单位名称的映射
unit_dict = {v: k for k, v in units.Neutral.__dict__.items() if isinstance(v, int)}  # 获取所有中立单位的 ID 和名称
# 更新字典，添加神族单位的 ID 到名称的映射
unit_dict.update({v: k for k, v in units.Protoss.__dict__.items() if isinstance(v, int)})
# 更新字典，添加人族单位的 ID 到名称的映射
unit_dict.update({v: k for k, v in units.Terran.__dict__.items() if isinstance(v, int)})
# 更新字典，添加虫族单位的 ID 到名称的映射
unit_dict.update({v: k for k, v in units.Zerg.__dict__.items() if isinstance(v, int)})
...
def get_img_obs_rgb(self, obs):
  """
  Extracts the RGB image from the observation, adds coordinate axes ranging from 0 to 128,
  marks the edges between black and other colors, and returns the Base64 encoded string of the processed image.
  If saving images is enabled in the configuration, the processed image is saved to a local file.
  """
  # Check the structure of the obs object
  if isinstance(obs, list):
  ...
  # Convert data type to uint8
  rgb_screen = rgb_screen.astype('uint8')
  # Convert NumPy array from BGR to RGB format
  rgb_screen = np.array(rgb_screen)[:, :, ::-1]  # BGR to RGB
  ...
  # Create a mask where pixels are black (all RGB values are zero)
  mask_black = np.all(img_array == [0, 0, 0], axis=-1)

  # Initialize an array to hold edges
  edges = np.zeros_like(mask_black, dtype=np.uint8)

  # Find edges between black and non-black regions
  edges[1:, :] |= mask_black[1:, :] != mask_black[:-1, :]  # Vertical edges
  edges[:, 1:] |= mask_black[:, 1:] != mask_black[:, :-1]  # Horizontal edges

  # Overlay edges onto the image by setting edge pixels to a color (e.g., red)
  edge_color = [255, 0, 0]  # Red color for edges
  img_array[edges > 0] = edge_color
...
# def get_img_obs_rgb(self, obs):
#     """
#     Extracts the RGB image from the observation, marks the edges between black and other colors,
#     and returns the Base64 encoded string of the processed image.
#     If saving images is enabled in the configuration, the processed image is saved to a local file.
#     """
#     # Check the structure of the obs object
